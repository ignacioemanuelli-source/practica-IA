...
export default function SessionForm({ onSaved }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!weight || !reps || !sets || weight <= 0 || reps <= 0 || sets <= 0) return;

    setSaving(true);
    setSuccess(false);

    // Se envía 'sets' junto con los demás campos para evitar el Error 400
  ...
  return (
    <div className="rounded-2xl border border-amber-500/30 bg-zinc-950/80 p-5 shadow-xl shadow-amber-500/5 backdrop-blur">
      ...
      <form onSubmit={handleSubmit}>
        {/* Layout en Grid de 3 columnas equivalente a la Calculadora */}
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          {/* Columna 1: Ejercicio y Carga */}
          ...
          {/* Columna 2: Series, Repeticiones y Fecha */}
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="mb-1 block text-xs text-zinc-400">Series</label>
                <input
                  type="number"
                  min={1}
                  placeholder="Ej. 3"
                  ...
                  onChange={(e) => {
                    ...
                    if (raw === '') {
                      ...
                      const parsed = Number(raw);
                      setSets(Number.isNaN(parsed) ? '' : parsed);
                    }
                  }}
                  required
                  className="w-full rounded-xl border border-zinc-800 bg-zinc-900 px-3 py-2 text-xs text-zinc-100 outline-none focus:border-amber-500"
              ...
              <div>
                <label className="mb-1 block text-xs text-zinc-400">Reps por serie</label>
                <input
                  type="number"
                  min={1}
                  placeholder="Ej. 10"
          ...
          {/* Columna 3: Confirmación y Botón de Acción */}
          <div className="flex flex-col justify-between rounded-xl border border-zinc-800/80 bg-zinc-900/60 p-4">
            <div className="space-y-1">
              <div className="flex items-center justify-between">
                <p className="text-[11px] font-medium text-amber-400">Resumen de Registro</p>
                <Calendar className="h-3.5 w-3.5 text-zinc-500" />
              </div>
              <p className="text-[10px] text-zinc-400">
                Registra tus series efectivas con técnica controlada para mantener un historial preciso.
              </p>
            </div>
